...
def input_values(folder, date, hour1, hour2, price_nok, win_in):
    ...
    price_nok_entry = Entry(Point(720, 150), 2)
    price_nok_entry.setText(price_nok)
    price_nok_entry.draw(win_in)
    ...
    price_nok = price_nok_entry.getText()
    ...
    if (a > 620 and a < 730) and (b > 389 and b < 410):
        ...
        plt.close('all')
        sys.exit()
    return folder, date, hour1, hour2, price_nok


def main():
    # Create initial window
    win_in = GraphWin("Control window", 800, 450)
    info_text1 = Text(Point(400, 35), "PLOT SPOT PRICE DATA FROM ENTSO-E")
    ...
    power_text1 = Text(Point(579, 150), "Price graph (Y/N):")
    ...
    while True:
        ...
        folder, date, hour1, hour2, price_nok = input_values(folder, date, hour1, hour2, price_nok, win_in)
        if int(hour1) < int(hour2) and int(hour1) >= 0 and int(hour1) <= 23 and int(hour2) >= 1 and int(hour2) <= 24:
            if price_nok == "Y" or price_nok == "y":
                plot_spot_0.main(folder + date[0:4] + date[5:7] + date[8:11] + "_NOK.data", hour1, hour2)
        if date == "2099-12-31":
            win_in.close()
            plt.close('all')
            sys.exit()
